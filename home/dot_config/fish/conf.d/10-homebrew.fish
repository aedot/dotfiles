...
# HOMEBREW_PREFIX is exported, so nested shells inherit it and skip the slow shellenv.
...
# Put the GNU coreutils/sed/etc. on PATH ahead of the BSD builtins.
if set -q HOMEBREW_PREFIX
    fish_add_path --global $HOMEBREW_PREFIX/opt/curl/bin
    fish_add_path --global $HOMEBREW_PREFIX/opt/gettext/bin

    for tool in coreutils grep gnu-sed findutils gnu-tar
